...
`opencode-threadflow` 是一个 OpenCode 插件项目。当前仓库已实现 `handoff`、`/resume-work`、`/search-session`、显式 session 引用、`find_session`、`read_session` 的 `preview` / `full` 分层输出，以及 `name_session` / `/name-session` 的 session 标题治理；核心目标仍是围绕 thread/session 的承接、引用、检索、恢复与命名可发现性。
...
│   ├── resume-work/              # /resume-work session 发现、context pack 组装与 hook 注入
...
- `./src/resume-work/AGENTS.md` - resume-work 领域运行时说明
...
| 插件入口               | `src/plugin.ts`                                                                                                                                     |
| 命令注册与模板         | `src/commands/index.ts`、`src/commands/handoff.ts`、`src/commands/session-search.ts`、`src/commands/name-session.ts`、`src/commands/resume-work.ts` |
| handoff 运行时实现     | `src/handoff/*`                                                                                                                                     |
| resume-work 运行时实现 | `src/resume-work/*`                                                                                                                                 |
| session 能力实现       | `src/sessions/*`                                                                                                                                    |
| 测试镜像               | `tests/handoff/*`、`tests/resume-work/*`、`tests/sessions/*`                                                                                        |
...
| 路径                                 | 用途                                                                                                           |
| ------------------------------------ | -------------------------------------------------------------------------------------------------------------- |
| `src/plugin.ts`                      | 创建 `createOpencodeClient`，组合 commands、sessions tools、sessions hooks、handoff hooks 与 resume-work hooks |
...
| `src/commands/resume-work.ts`        | `/resume-work` 命令名常量                                                                                      |
...
| `src/resume-work/index.ts`           | resume-work 域 hook 注册入口，`command.execute.before` 注入编排                                                |
| `src/resume-work/context-builder.ts` | 近期 session 发现、full context pack 组装与 synthetic 注入文本渲染                                             |
...
| `tests/resume-work/*`                | `/resume-work` 的 context builder 与 command injection 行为测试                                                |
...
- `/resume-work` 是 `find_session`（空 query）+ `read_session`（full）的薄层组合，零参数，纯时间局部性，不实现独立的 session 发现/精炼逻辑；注入在 `command.execute.before` 完成，synthetic text 标记 `synthetic: true`。
...
- 不要把 resume-work 逻辑塞入 `src/sessions/` 或 `src/handoff/`。
- 不要让 `/resume-work` 支持关键词过滤——这会破坏纯时间局部性设计，与 `/search-session` 定位重叠。
